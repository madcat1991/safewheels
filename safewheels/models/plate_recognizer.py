...
class PlateRecognizer:
    ...
    def __init__(self, languages=None, gpu=True):
        ...
        self.plate_patterns = {
            'usa': r'[A-Z0-9]{3,8}',
            'european': r'[A-Z]{1,3}[-\s]?[0-9]{1,4}[-\s]?[A-Z0-9]{1,3}',
            'german': r'[A-Z]{1,3}[-\s]?[A-Z]{1,2}[-\s]?[0-9]{1,4}',
    ...
    def _preprocess_plate(self, plate_img):
        ...
        try:
            ...
            resized = cv2.resize(plate_img, (new_width, new_height), interpolation=cv2.INTER_AREA)  # AREA for downsampling
    ...
    def _clean_plate_text(self, text):
        ...
        # German plates typically start with 1-3 letters (city code)
        if re.match(r'^[A-Z]{1,3}', text):
            # This is likely a German plate, apply city code corrections
            first_part_match = re.match(r'^([A-Z]{1,3})', text)
            if first_part_match:
                first_part = first_part_match.group(1)
                rest_of_text = text[len(first_part):]
                
                # Convert specific numbers to letters in the city code
                for num, letter in corrections.items():
                    first_part = first_part.replace(num, letter)
    ...
    def recognize_characters(self, plate_img, confidence_threshold=0.3):
        ...
        try:
            ...
            for img in processed_images:
                try:
                    ...
                    detection_results1 = self.reader.readtext(
                        ...
                        allowlist='ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789',  # Limit to Latin plate characters
            ...
            logger.debug(f"Best plate candidate: {best_result[0]} (conf: {best_result[1]:.2f}, method: {best_result[2]})")
